...
func (b *Bridge) wrapTxBuilder(rawTx *BuildRawTx) error {
	if rawTx.TxBuilder != nil {
		return nil
	}
	if len(rawTx.EncodedTx) == 0 {
		return fmt.Errorf("cannot wrap tx builder from rawtx")
	}
	tx, err := b.TxConfig.TxDecoder()(rawTx.EncodedTx)
	if err != nil {
		return err
	}
	txBuilder, err := b.TxConfig.WrapTxBuilder(tx)
	if err != nil {
		return err
	}
	rawTx.TxBuilder = txBuilder
	return nil
}
